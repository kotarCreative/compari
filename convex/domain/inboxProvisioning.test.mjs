...
test('inbox transitions are explicit and reject stale completion', () => {
  assert.equal(transitionInboxProvisioning('pending', 'provisioning'), 'provisioning')
  assert.equal(transitionInboxProvisioning('provisioning', 'ready'), 'ready')
  assert.throws(() => transitionInboxProvisioning('ready', 'provisioning'), DomainTransitionError)
})
...
test('external failures expose safe, actionable categories', () => {
  assert.deepEqual(safeExternalError(new Error('429 timeout')), { category: 'retryable_external', summary: 'Inbox provider is temporarily unavailable.' })
  assert.deepEqual(safeExternalError(new Error('401 invalid API key')), { category: 'permanent_external', summary: 'Inbox provider credentials need attention.' })
})

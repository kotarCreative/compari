...
import { canClaimJob, canRetryJob, retryDelayMs } from './domain/sideEffectPolicy'
...
export const claim = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch('sideEffectJobs', job._id, {
      status: 'running', claimToken, claimedAt: now, leaseExpiresAt: now + 60_000,
...
export const retryOrFail = internalMutation({
  args: { jobId: v.id('sideEffectJobs'), claimToken: v.string(), summary: v.string(), retryable: v.boolean() },
  returns: v.union(v.null(), v.object({ retryAt: v.optional(v.number()) })),
  handler: async (ctx, args) => {
    const job = await ctx.db.get('sideEffectJobs', args.jobId)
    if (!job || job.status !== 'running' || job.claimToken !== args.claimToken) return null
    const now = Date.now()
    const canRetry = canRetryJob({ retryable: args.retryable, attemptCount: job.attemptCount, maxAttempts: job.maxAttempts })
    ...
    await ctx.db.patch('sideEffectJobs', job._id, {
      ...
      lastErrorSummary: args.summary.slice(0, 300), scheduledAt: retryAt,
      claimToken: undefined, leaseExpiresAt: undefined, updatedAt: now,
    })
...
export const scheduleRetry = internalMutation({
  ...
  handler: async (ctx, args) => {
    const job = await ctx.db.get('sideEffectJobs', args.jobId)
    if (!job || job.status !== 'retryable_failure' || job.scheduledAt !== args.retryAt)
      return null
    const delay = Math.max(0, args.retryAt - Date.now())
    switch (job.kind) {
      case 'extract_requirements':
        if (job.requestId)
          await ctx.scheduler.runAfter(delay, internal.workflows.extractRequirements, {
            requestId: job.requestId, jobId: job._id,
          })
        break
      case 'discover_providers':
        if (job.requestId)
          await ctx.scheduler.runAfter(delay, internal.workflows.discoverProviders, {
            requestId: job.requestId, jobId: job._id,
          })
        break
      case 'research_candidate':
        if (job.candidateId)
          await ctx.scheduler.runAfter(delay, internal.workflows.researchCandidate, {
            candidateId: job.candidateId, jobId: job._id,
          })
        break
      case 'rank_candidates':
        if (job.requestId)
          await ctx.scheduler.runAfter(delay, internal.rankingWorkflow.rank, { requestId: job.requestId, jobId: job._id })
        break
      case 'send_outreach_email':
      case 'submit_contact_form':
        if (job.outreachAttemptId)
          await ctx.scheduler.runAfter(delay, internal.outreachWorkflow.execute, {
            attemptId: job.outreachAttemptId, jobId: job._id,
          })
        break
      case 'evaluate_request':
        if (job.requestId)
          await ctx.scheduler.runAfter(delay, internal.evaluationsWorkflow.generate, {
            requestId: job.requestId, jobId: job._id,
          })
        break
      case 'send_follow_up':
        if (job.questionId)
          await ctx.scheduler.runAfter(delay, internal.followUps.sendForAnsweredQuestion, {
            questionId: job.questionId, jobId: job._id,
          })
        break
      default:
        return null

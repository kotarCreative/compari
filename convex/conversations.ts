...
function conversationSummary(conversation: Doc<'conversations'>) {
  return {
...
function messageSummary(message: Doc<'providerMessages'>) {
  return {
    _id: message._id,
...
export const get = query({
  ...
  returns: v.object({
    ...
    messages: paginationResultValidator(messageValidator),
  }),
  handler: async (ctx, args) => {
    const conversation = await ctx.db.get('conversations', args.conversationId)
    if (!conversation)
      throw new Error('validation: conversation does not exist')
    ...
    return {
      ...
      messages: { ...messages, page: messages.page.map(messageSummary) },
    }
  },
})
...
export const resolveRouting = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!event || !candidate)
      throw new Error('validation: routing target does not exist')
    const { request, user } = await requireOwnedRequest(
      ctx,
      candidate.requestId,
    )
    if (event.status !== 'needs_user')
      throw new Error('validation: routing has already been resolved')
    if (!event.inboxId || event.inboxId !== user.agentMailInboxId)
      throw new Error(
        'authorization: webhook inbox does not belong to this buyer',
      )
    const byThread = event.threadId
      ? await ctx.db
          .query('conversations')
          .withIndex('by_agent_mail_thread_id', (q) =>
            q.eq('agentMailThreadId', event.threadId!),
          )
          .unique()
      : null
    if (
      byThread &&
      (byThread.requestId !== request._id ||
        byThread.candidateId !== candidate._id)
    )
      throw new Error(
        'validation: webhook thread belongs to a different request',
      )
    await ctx.db.patch('webhookEvents', event._id, {
      proposedCandidateId: candidate._id,
      status: 'pending',
      safeError: undefined,
    })
...
export const ingestProviderMessage = internalMutation({
  ...
  handler: async (ctx, args) => {
    const recordedEvent = await ctx.db
      .query('webhookEvents')
    ...
    if (
      recordedEvent &&
      recordedEvent.status !== 'pending' &&
      recordedEvent.status !== 'running' &&
      recordedEvent.status !== 'retryable_failure'
    )
      return null
    const now = Date.now()
    const eventId = recordedEvent
      ? recordedEvent._id
      : await ctx.db.insert('webhookEvents', {
          ...
          proposedCandidateId: args.candidateId,
          receivedAt: now,
        })
    ...
    const candidate = args.candidateId
      ? await ctx.db.get('requestCandidates', args.candidateId)
      : null
    if (
      conversation &&
      candidate &&
      (conversation.candidateId !== candidate._id ||
        conversation.requestId !== candidate.requestId)
    ) {
      await ctx.db.patch('webhookEvents', eventId, {
        status: 'needs_user',
        safeError: 'Thread and proposed candidate do not match.',
      })
      return null
    }
    if (!conversation && candidate) {
      ...
      if (request && user && user.agentMailInboxId === args.inboxId) {
        conversation = await ctx.db
          ...
          .then((id) => ctx.db.get('conversations', id))
      }
    }
    if (!conversation || conversation.inboxId !== args.inboxId) {
      await ctx.db.patch('webhookEvents', eventId, {
        status: 'needs_user',
        safeError: 'Routing needs buyer confirmation.',
      })
      return null
    }
    const routedCandidate = await ctx.db.get(
      'requestCandidates',
      conversation.candidateId,
    )
    const routedRequest = await ctx.db.get(
      'procurementRequests',
      conversation.requestId,
    )
    if (!routedCandidate || !routedRequest) {
      await ctx.db.patch('webhookEvents', eventId, {
        status: 'needs_user',
        safeError: 'The routed request or provider no longer exists.',
      })
      return null
    }
    const messageId = await ctx.db.insert('providerMessages', {
      conversationId: conversation._id,
      direction: 'inbound',
      participants: [args.sender.slice(0, 320)],
      subject: args.subject.slice(0, 500),
      sanitizedBody: args.body.slice(0, 40_000),
      agentMailMessageId: args.messageId,
      attachments: { schemaVersion: 1, value: args.attachments ?? [] },
    ...
    const extractionJobId = await ctx.db.insert('sideEffectJobs', {
      userId: routedRequest.userId,
      kind: 'extract_provider_response',
      idempotencyKey: `extract-provider-response:${messageId}:v1`,
      status: 'pending',
      attemptCount: 0,
      maxAttempts: 3,
      requestId: routedRequest._id,
      candidateId: routedCandidate._id,
      inputVersion: 1,
      scheduledAt: now,
      createdAt: now,
      updatedAt: now,
    ...
    await ctx.scheduler.runAfter(
      ...
      { providerMessageId: messageId, jobId: extractionJobId },
    )
    return messageId
  },
})
...
export const reconcileDeliveryEvent = internalMutation({
  ...
  handler: async (ctx, args) => {
    const attempt = await ctx.db
      ...
      .withIndex('by_vendor_message_id', (q) =>
        q.eq('vendorMessageId', args.messageId),
      )
      .unique()
    ...
    await ctx.db.patch('outreachAttempts', attempt._id, {
      status: 'needs_user',
      safeError:
        'Provider mail delivery reported a bounce, complaint, or rejection.',
      updatedAt: now,
    })
    return null
  },

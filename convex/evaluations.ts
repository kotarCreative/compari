...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    const evaluation = await ctx.db
      .query('evaluations')
      .withIndex('by_request_id_and_input_version', (q) =>
        q.eq('requestId', args.requestId).eq('inputVersion', inputVersion),
      )
      .unique()
    ...
    return {
      ...
      views: views.map((view) => ({
        ...
        inputVersion: view.inputVersion,
      })),
    }
  },
})
...
export const reconfigure = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert('activityEvents', {
      requestId: request._id,
      eventType: 'evaluation_reconfigured',
      safeMessage:
        'Comparison preferences were updated; facts and requirements were unchanged.',
...
export const loadSnapshot = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      job.inputVersion !== (request.evaluationInputVersion ?? request.version)
    )
      return null
    ...
    return {
      candidates: hydrated
        ...
        .map(({ id, name }) => ({ id, name })),
      metrics: [
        { key: 'price', label: 'Quoted price', kind: 'number' as const },
        { key: 'availability', label: 'Availability', kind: 'text' as const },
        {
          key: 'confidence',
      ...
      preference: request.evaluationPreference,
      requirementCount: requirements.length,
      factCount: facts.length,
      proposalCount: proposals.length + (hasPrice && hasAvailability ? 0 : 0),
    }
  },
})
...
export const apply = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const facts = await ctx.db
      ...
      .withIndex('by_request_id', (q) => q.eq('requestId', request._id))
      .take(200)
    const missing = candidateRows.length
      ? [
          'Confirm scope, exclusions, and availability against the cited website price or latest provider response.',
        ]
      : ['No viable website price or provider response is available.']
    await ctx.db.insert('evaluations', {
      requestId: request._id,
      inputVersion: inputVersion ?? request.version,
      criteria: { schemaVersion: 1, value: metrics },
      results: {
        schemaVersion: 1,
        value: {
          candidateIds,
          ...
          caveats: missing,
          preference: request.evaluationPreference ?? null,
        },
      },
      recommendation: args.recommendation.slice(0, 1_500),
      generatedAt: now,
    })
    for (const view of views)
      await ctx.db.insert('uiViews', {
        requestId: request._id,
        label: view.label,
        viewType: view.type,
        configuration: { schemaVersion: 1, value: view },
        inputVersion: inputVersion ?? request.version,
        generatedAt: now,
      })

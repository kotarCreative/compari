const email = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
const commitment =
  /\b(accept|agree|book|purchase|pay(?:ment)?|sign|contract)\b/i
...
export function validateOutboundPreflight(
  input: OutboundPreflight,
): string | null {
  if (
    input.paused ||
    !['researching', 'contacting'].includes(input.requestStatus)
  )
  ...
  if (
    !input.candidateQueued ||
    !input.endpointVerified ||
    input.previousSucceeded
  )
  ...
  if (input.endpointType !== 'email')
    return 'contact-form automation is not enabled'
  if (!email.test(input.endpointValue)) return 'email endpoint is invalid'
  if (
    !input.subject.trim() ||
  ...
  if (
    secret.test(`${input.subject}\n${input.body}`) ||
    personalData.test(input.body)
  )
...
export function classifyExternalError(error: unknown): {
  ...
  return {
    retryable:
      ...
      /\b(timeout|rate limit|temporar)/i.test(summary),
